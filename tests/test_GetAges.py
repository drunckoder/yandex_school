...
from tests.testing_utils import make_citizen, send_create_import_request, \
    send_get_ages_request, send_get_citizens_request
...
def test_bad_import_id(client):
    import_id = -1
    status, data = send_get_ages_request(client, import_id)
    assert status == 404
...
def test_basic_test(client):
    ...
    status, data = send_get_ages_request(client, import_id=import_id)
    assert status == 200


def test_basic_test_2(client):
    citizens = [make_citizen(citizen_id=x, town='x') for x in range(1, 11)]
    body = {'citizens': citizens}
    status, data = send_create_import_request(client, body)
    assert status == 201
